...
class TestRouter(TestCase):
    def setUp(self):
        router.TRACKED_CLIENTS = ['130.10.20.24', '130.10.20.25']
        router.TOR_PEERS = ['99.10.20.30', '128.0.0.128']
    ...
    def test_client_is_tracked_and_marked(self):
        src_ip = str_to_inet(router.TRACKED_CLIENTS[0])
        dst_ip = str_to_inet(router.TOR_PEERS[0])
        first_ip_packet = IP(id=0, src=src_ip, dst=dst_ip, p=IP_PROTO_TCP)
        first_ip_packet.data = TCP(sport=1024, dport=2048, data=b'\x05\x04\x03\x02\x01')
